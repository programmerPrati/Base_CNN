...
def dropout_backward(dout, cache):
  ...
  if mode == 'train':
    ...
    #pass
    dx = dout * mask # just multiply with the mask for backprop
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  # Extract dimensions and hyperparameters
  N, C, H, W = x.shape
  F, c, HH, WW = w.shape
  ...
  # Perform convolution
  for i in range(N):          # Loop over each input
      for j in range(F):       # Loop over each filter
          for h in range(H_out):  # Loop over output height
              for k in range(W_out):  # Loop over output width
                  ...
                  # Compute the convolutional operation
                  out[i, j, h, k] = np.sum(x_padded[i, :, h_start:h_end, w_start:w_end] * w[j, :, :, :]) + b[j]
...
def conv_backward_naive(dout, cache):
  ...
  # Pad x and dx for the padding added in the forward pass
  x_padded = np.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)), mode='constant')
  ...
  for i in range(N):  # Loop over each image
      for j in range(F):  # Loop over each filter
          for h in range(H_out):
              for k in range(W_out):
                  ...
                  # dw: accumulate gradients for the filter weights
                  dw[j] += x_padded[i, :, h_start:h_end, w_start:w_end] * dout[i, j, h, k]

                  # dx_padded: accumulate gradients for the input
                  dx_padded[i, :, h_start:h_end, w_start:w_end] += w[j] * dout[i, j, h, k]
...
def max_pool_forward_naive(x, pool_param):
  ...
  for i in range(H_out):
      for j in range(W_out):
          ...
          # Apply max pooling over the window for each channel and each input
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in range(H_out):
      for j in range(W_out):
          # Define the window in the input corresponding to the current output
          h_start = i * stride
          h_end = h_start + pool_height
          w_start = j * stride
          w_end = w_start + pool_width
          
          # Iterate over each data point and channel
          for n in range(N):
              for c in range(C):
                  ...
                  # Create a mask for the max value position
                  mask = (window == max_val)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  # Reshape x to (N*H*W, C) to perform batch normalization across the channel dimension
...
def spatial_batchnorm_backward(dout, cache):
  ...
  # Reshape dout to (N * H * W, C) for vanilla batch normalization backward
